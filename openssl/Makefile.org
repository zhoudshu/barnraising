...
SHLIB_MARK= $(SHARED_LIBS)

DIRS=   crypto ssl rsaref $(SHLIB_MARK) apps doc tools
...
do_gnu-shared:
	...
	libs="$$libs -l$$i"; \
	( set -x; \
		ln -sf lib$$i.so.${SHLIB_MAJOR}.${SHLIB_MINOR} lib$$i.so ); \
	done
...
clean:
	...
	rm -f *.a *.o speed.* *.map .pure core
#	rm -f *.so.* *.old util/*.old
	rm -f *.old util/*.old
	rm -f $(TARFILE)
	@for i in $(ONEDIRS) ;\
	do \
...
rehash: rehash.time
rehash.time: certs
	@(OPENSSL="`pwd`/apps/openssl"; export OPENSSL; LD_LIBRARY_PATH=$(TOP)/; export LD_LIBRARY_PATH; $(PERL) tools/c_rehash certs)
	touch rehash.time
...
tests: rehash
	...
	@(LD_LIBRARY_PATH=$(TOP)/ apps/openssl version -a
...
install: all install_docs
	...
	do \
		if [ -f "$$i" ]; then \
		(       echo installing $$i; \
			cp -d $$i $(INSTALL_PREFIX)$(INSTALLTOP)/lib; \
...
install_docs:
	...
	for i in doc/apps/*.pod; do \
		...
		echo "installing man$$sec/`basename $$i .pod`.$$secsuf"; \
		(cd `$(PERL) util/dirname.pl $$i`; \
		sh -c "$(PERL) $$pod2man \
			--section=$$secsuf --center=OpenSSL \
			--release=$(VERSION) `basename $$i`") \
			>  $(INSTALL_PREFIX)$(MANDIR)/man$$sec/`basename $$i .pod`.$$secsuf; \
	done; \
